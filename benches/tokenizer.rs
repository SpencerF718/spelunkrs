...
fn benchmark_tokenize(criterion: &mut Criterion) {
    ...
    let input = "你好".repeat(5_000);

    criterion.bench_function("tokenize repetition-heavy", |bencher| {
        bencher.iter(|| tokenizer.tokenize(&input));
    });

    let qian_zi_wen_input = QIAN_ZI_WEN.repeat(10);
